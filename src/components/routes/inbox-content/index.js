...
import { fetchInboxStatistic, getInboxUsageStatistics } from './actions'
...
class Inboxcontent extends Component<PropsMain, StateMain> {
    ...
    componentDidMount(){
        this.props.getInboxUsageStatistics();
    }


    accordionClick = (InboxcontentId: string) => () => {

        if (this.state.openMessage === InboxcontentId) {
            this.setState({
                openMessage: null
            });
        } else {
            this.setState({
                openMessage: InboxcontentId
            });
        }
    };
    ...
    render() {
        ...
        const listItems = this.props.inboxContent.data.map(item =>
...
export default connect(state => ({ inboxContent: state.inboxContent} ),{ fetchInboxStatistic, getInboxUsageStatistics})(Inboxcontent);