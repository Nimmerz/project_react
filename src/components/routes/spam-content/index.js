...
import { fetchSpamStatistic, getSpamUsageStatistics } from './actions'
...
class Spamcontent extends Component<PropsMain, StateMain> {
    ...
    componentDidMount(){
        this.props.getSpamUsageStatistics();
    }

    accordionClick = (SpamcontentId: string) => () => {

        if (this.state.openMessage === SpamcontentId) {
            this.setState({
                openMessage: null
            });
        } else {
            this.setState({
                openMessage: SpamcontentId
            });
        }
    };
    ...
    render() {
        ...
        const listItems = this.props.spamContent.data.map(item =>
...
export default connect(state => ({ spamContent: state.spamContent} ),{ fetchSpamStatistic, getSpamUsageStatistics})(Spamcontent);
